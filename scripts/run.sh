if [ $# -gt 0 ]; then
  # Set duration to the first argument
  duration=$1
  # use local network if provided second argument (anything is okay)
  if [ $# -gt 1 ]; then
    public="false"
  else
    public="true"
  ...
  # No argument: duration for 30 seconds, also using public network
  duration=30
  public="true"
fi
...
./run_single.sh pbft $count $host_ip $duration
./run_single.sh zyzzyva $count $host_ip $duration
